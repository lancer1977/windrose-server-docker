...
| native-plugin-3 shared runtime plumbing | Docs and tests cover plugin manifest/status/dry-run/smoke-options plus V3 execute queue/result readback; `windrose2-dev` proved plugin consumption with `nativeSpawn=false`. | `GET /api/plugin/smoke-options`, `POST /api/plugin/actions/execute`, `GET /api/plugin/actions/{actionRequestId}/result`, `tests/Windrose.StateWeb.Tests/Api/WindroseEndpointsTests.cs`, `plugins/windrose-sidecar-bridge/README.md` |
| native-plugin-4 first visible server-side action | Deferred in the current docs; V3 queue/readback is not native in-game spawning. Broadcast/chat/actor spawn remains a native-hook or future-upstream problem, not a stable Lua API. | `docs/roadmaps/windrose-runtime-control-surface/README.md`, `docs/roadmaps/windrose-runtime-control-surface/operator-contract.md`, `docs/roadmaps/windrose-runtime-control-surface/possibility-atlas.md` |
